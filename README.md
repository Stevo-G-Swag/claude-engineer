...
Claude Engineer is now available as a VS Code extension, providing an integrated development experience directly within your favorite code editor.
...
4. Open the `claude-engineer` directory in VS Code:
   ```
   code .
   ```

5. Press `F5` to start debugging the extension. This will open a new VS Code window with the extension loaded.

### Usage

Once the extension is loaded, you can use the voice command palette to execute common commands using voice input.

1. Press `Ctrl+Shift+P` to open the command palette.
2. Type `Voice Command Palette` and press `Enter`.
3. Speak your command into the microphone.

Supported voice commands:
- "exit voice mode": Exits the voice mode.
- "save chat": Saves the current chat log.
- "reset conversation": Resets the conversation history.

The extension provides feedback and confirmation for voice commands using VS Code's notification API.

### Enhanced Voice Mode

The VS Code extension includes an enhanced voice mode with the following features:

- 🗣️ Voice command palette for executing common commands using voice input.
- 🎙️ Real-time speech-to-text conversion using the `speech_recognition` library.
- 🔄 Feedback and confirmation for voice commands using VS Code's notification API.

To use the enhanced voice mode, follow the installation and usage instructions above.
